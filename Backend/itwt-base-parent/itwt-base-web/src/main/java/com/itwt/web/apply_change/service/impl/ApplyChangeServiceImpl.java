package com.itwt.web.apply_change.service.impl;
...
import com.itwt.web.apply_change.mapper.ApplyChangeMapper;
    ...
    public void applyDo(ApplyDoParm change) {
        ...
        if(change.getType().equals("1")){
            ...
            if(i>0){
                ...
                //删除原来的
                LambdaQueryWrapper<StuBed> applyQuery = new LambdaQueryWrapper<>();
                applyQuery.eq(StuBed::getStuId,applyUserId);
                stuBedService.remove(applyQuery);
                LambdaQueryWrapper<StuBed> changeQuery = new LambdaQueryWrapper<>();
                changeQuery.eq(StuBed::getStuId,changeUserId);
