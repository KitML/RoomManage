package com.itwt.web.drom_leave.controller;
...
import com.itwt.web.drom_leave.entity.LeaveParm;
...
public class DromLeaveController {
    ...
    public ResultVo list(LeaveParm parm){
        ...
        //构造查询条件
        LambdaQueryWrapper<DromLeave> queryWrapper = new LambdaQueryWrapper<>();
        if (StringUtils.isNotEmpty(parm.getClasses())){
            queryWrapper.like(DromLeave::getClasses,parm.getClasses());
        }
        if (StringUtils.isNotEmpty(parm.getDromName())){
            queryWrapper.like(DromLeave::getDromName,parm.getDromName());
        }
        if (StringUtils.isNotEmpty(parm.getStuName())){
            queryWrapper.like(DromLeave::getStuName,parm.getStuName());
        }
        IPage<DromLeave> list = dromLeaveService.page(page,queryWrapper);
